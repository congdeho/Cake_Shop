    ...
    const cities = [
        {
            ...
            districts: [
                ...
                "Quận 10",
                "Quận Bình Thạnh",
                "Quận Gò Vấp",document.addEventListener("DOMContentLoaded", function () {
  const cities = [
    {
      name: "Thành phố Hồ Chí Minh",
      districts: [
...
function capNhatMoiThu() {
  ...
  document.querySelector(".cart-number").textContent =
    getCurrentUser().products.length;
}
function toggleMenu() {
...
// Handle form submission and payment method
document
  ...
  .addEventListener("submit", function (event) {
    ...
    closePaymentModal(); // Close the modal after payment confirmation
  });
function handlePayment() {
  ...
  alert(
    `Bạn đã chọn phương thức thanh toán: ${selectedPaymentMethod}\nTổng thanh toán: ${totalPrice} VND`
  );
}
